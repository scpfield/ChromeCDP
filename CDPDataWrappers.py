...
class CDPEvent():
    ...
    def PrintMessage( self ):
        ...
        Output += ( self.DataColor + 
                    json.dumps( self.Message, indent = 2 ) +
                    self.ResetColor + '\n' )
        Output += '\n'
        print( Output, Truncate=None, Decorate=False)
    ...
    def PrintToScreen( self ):
        ...
        if not self.OutputScreen.ScreenThread.is_alive():
            self.PrintMessage()
            return True
...
class CDPReturnValue():
    ...
    def PrintCommand( self, **kwargs ):
        Output = '\n'
        Output += ( self.LabelColor + "Command: " + 
                    self.ResetColor + '\n' )
        Output += ( self.DataColor + 
                    json.dumps( self.Command, indent = 2 ) +
                    self.ResetColor + '\n' )
        Output += '\n'
        print( Output, **kwargs)
    
    def PrintError( self, **kwargs ):
        Output = '\n'
        Output += ( self.ErrorLabelColor + "Error Result: " + 
                    self.ResetColor + '\n' )
        Output += ( self.ErrorColor + 
                    json.dumps( self.Error, indent = 2 ) +
                    self.ResetColor + '\n' )
        Output += ( "Failed Command: \n" )
        Output += ( json.dumps( self.Command, indent = 2 ) )
        Output += '\n'
        print( Output, **kwargs)
    
    def Print( self, **kwargs ):
        self.PrintCommand(**kwargs)
        if self.Error == None:
            self.PrintResult(**kwargs)
        else:
            self.PrintError(**kwargs)
    
    def PrintToFile( self, FileName = None, Mode = None):
        
        if not FileName: FileName = 'CDPResult.json'
        if not Mode: Mode = 'a'
        ...
        if self.Error:
            ErrorFileName = FileName.replace('.', '-Error.')
            with open( ErrorFileName, 'a' ) as ErrorFile:
                print('Dumping Error info to: ', ErrorFileName )
                json.dump( self.Error, ErrorFile, indent = 2 )

        if self.Result:
            with open( FileName, Mode ) as ResultFile:
                print( "Dumping Result info to: ", FileName )
                json.dump( self.Result, ResultFile, indent = 2 )
    ...
    def __len__( self ):
        if self.Response:
            return( len( json.dumps( self.Response, indent = 2 )))
    
    def __getitem__(self, Index):
        if self.Response:
            return( json.dumps( self.Response, indent = 2 )[Index])    
